...
// make the below the same value
String sDeltaAngle = "90"; // initial servo position
int angle = 90; 
...
void serialServo()
{
  ...
  while(true)  // "true" is always true, so this will loop forever.
  {
    ...
    while (Serial.available() > 0)
    {
      ...
      // SCD: test this logic by typing alpha numeric and nominal characters
      sDeltaAngle = Serial.readStringUntil('\r');
      angle = 90 + sDeltaAngle.toInt();
      // int dDeltaAngleMax = 3;
      // angle = angle + constrain(sDeltaAngle.toInt(), -dDeltaAngleMax, dDeltaAngleMax);
